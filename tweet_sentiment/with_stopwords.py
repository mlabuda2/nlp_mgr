...
# Encode labels in column 'reviews_score'.
...
unknown_words_in_test_after_remove_duplicated_letters = set()
...
if chosen_tokenizer == 'keras':
    ...
    for word, index in tokenizer.word_index.items():
        ...
        if embedding_vector is not None:
            ...
            # jeśli nie znaleźliśmy takiego słowa w ED to spróbujmy to: hhhaaallloo -> halo
            word = ''.join(letter_group[0] for letter_group in itertools.groupby(word))
            embedding_vector = embeddings_dictionary.get(word)
            if embedding_vector is not None:
                embedding_matrix[index] = embedding_vector
            else:
                unknown_words_in_test_after_remove_duplicated_letters.add(word)
    # print("Ilość unik. nauczonych słów z train:", len(all_words_in_test))
    print("Nieznane słowa z test:", len(unknown_words_in_test))
    print("Nieznane słowa z test 2:", len(unknown_words_in_test_after_remove_duplicated_letters))
    ###########
    # """
...
if chosen_tokenizer == 'keras':
    history = model.fit(X1_train, y_train, batch_size=128, epochs=10, verbose=1, validation_split=0.2)
